...
# Annamaria Mesaros, Sharath Adavanne, Archontis Politis, Toni Heittola,
# Tuomas Virtanen
# WASPAA 2019
#
#
                ...
                if class_cnt in gt[block_cnt] and class_cnt in pred[block_cnt]:
                    ...
                    for gt_ind, gt_val in enumerate(gt_ind_list):
                        if gt_val in pred_ind_list:
                            ...
                            gt_arr = np.array(
                                gt[block_cnt][class_cnt][0][1][gt_ind])
                            pred_arr = np.array(
                            ...
                            if gt_arr.shape[0] == 1 and pred_arr.shape[0] == 1:
                                total_spatial_dist += distance_between_cartesian_coordinates(
                                    gt_arr[0][0], gt_arr[0][1], gt_arr[0][2], pred_arr[0][0], pred_arr[0][1], pred_arr[0][2])
                            else:
                ...
                if class_cnt in gt_deg[block_cnt] and class_cnt in pred_deg[block_cnt]:
                    ...
                    for gt_ind, gt_val in enumerate(gt_ind_list):
                        if gt_val in pred_ind_list:
                            ...
                            gt_arr = np.array(
                                gt_deg[block_cnt][class_cnt][0][1][gt_ind]) * np.pi / 180
                            pred_arr = np.array(
                                pred_deg[block_cnt][class_cnt][0][1][pred_ind]) * np.pi / 180
                            if gt_arr.shape[0] == 1 and pred_arr.shape[0] == 1:
                                total_spatial_dist += distance_between_spherical_coordinates_rad(
                                    gt_arr[0][0], gt_arr[0][1], pred_arr[0][0], pred_arr[0][1])
                            else:
                                total_spatial_dist += least_distance_between_gt_pred(
                                    gt_arr, pred_arr)
...
"""
    ...
    if gt_len and pred_len:
        if len(gt_list[0]) == 3:  # Cartesian
            x1, y1, z1, x2, y2, z2 = gt_list[ind_pairs[:, 0], 0], gt_list[ind_pairs[:, 0], 1], gt_list[ind_pairs[:,
                                                                                                                 0], 2], pred_list[ind_pairs[:, 1], 0], pred_list[ind_pairs[:, 1], 1], pred_list[ind_pairs[:, 1], 2]
            cost_mat[ind_pairs[:, 0], ind_pairs[:, 1]] = distance_between_cartesian_coordinates(
                x1, y1, z1, x2, y2, z2)
        else:
            az1, ele1, az2, ele2 = gt_list[ind_pairs[:, 0], 0], gt_list[ind_pairs[:,
                                                                                  0], 1], pred_list[ind_pairs[:, 1], 0], pred_list[ind_pairs[:, 1], 1]
