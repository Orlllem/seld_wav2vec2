...
class SELDMetrics(object):
    ...
    def f1_overall_1sec(self, O, T):
        ...
        for i in range(0, new_size):
            O_block[i, :] = np.max(
                O[int(i * self._block_size):int(i * self._block_size + self._block_size - 1), :], axis=0)
            T_block[i, :] = np.max(
                T[int(i * self._block_size):int(i * self._block_size + self._block_size - 1), :], axis=0)
    ...
    def update_doa_scores(self, pred_doa_thresholded, gt_doa):
        ...
        for frame in range(pred_doa_thresholded.shape[0]):
            ...
            if nb_gt_peaks and nb_pred_peaks:
                ...
                pred_list_rad = np.array(
                    self._data_gen .get_matrix_index(pred_ind)) * np.pi / 180

                gt_ind = np.where(gt_doa[frame] == 1)[1]
                gt_list_rad = np.array(
                    self._data_gen .get_matrix_index(gt_ind)) * np.pi / 180

                frame_dist = distance_between_gt_pred(
...
def f1_overall_1sec(O, T, block_size):
    ...
    for i in range(0, new_size):
        O_block[i, :] = np.max(
            O[int(i * block_size):int(i * block_size + block_size - 1), :], axis=0)
        T_block[i, :] = np.max(
            T[int(i * block_size):int(i * block_size + block_size - 1), :], axis=0)
...
def compute_doa_scores_regr_xyz(pred_doa, gt_doa, pred_sed, gt_sed):
    ...
    for frame_cnt, sed_frame in enumerate(gt_sed):
        ...
        if nb_src_gt_list[frame_cnt] == nb_src_pred_list[frame_cnt]:
            ...
            less_est_cnt = less_est_cnt + \
                nb_src_gt_list[frame_cnt] - nb_src_pred_list[frame_cnt]
            less_est_frame_cnt = less_est_frame_cnt + 1
        elif nb_src_gt_list[frame_cnt] < nb_src_pred_list[frame_cnt]:
            more_est_cnt = more_est_cnt + \
                nb_src_pred_list[frame_cnt] - nb_src_gt_list[frame_cnt]
        ...
        if nb_src_gt_list[frame_cnt] and nb_src_pred_list[frame_cnt]:
            ...
            doa_frame_gt_y = gt_doa[frame_cnt][nb_sed:2 *
                                               nb_sed][sed_frame_gt == 1]
            ...
            doa_frame_pred_y = pred_doa[frame_cnt][nb_sed:2 *
                                                   nb_sed][sed_frame_pred == 1]
            doa_frame_pred_z = pred_doa[frame_cnt][2 *
                                                   nb_sed:][sed_frame_pred == 1]
    ...
    er_metric = [doa_loss_pred, frame_recall, doa_loss_pred_cnt,
                 good_frame_cnt, more_est_cnt, less_est_cnt]
    return er_metric
...
def compute_doa_scores_clas(pred_doa_thresholded, gt_doa, data_gen_test):
    ...
    for frame in range(pred_doa_thresholded.shape[0]):
        ...
        if nb_gt_peaks and nb_pred_peaks:
            pred_ind = np.where(pred_doa_thresholded[frame] == 1)[1]
            pred_list_rad = np.array(
                data_gen_test.get_matrix_index(pred_ind)) * np.pi / 180

            gt_ind = np.where(gt_doa[frame] == 1)[1]
            gt_list_rad = np.array(
                data_gen_test.get_matrix_index(gt_ind)) * np.pi / 180
    ...
    er_metric = [doa_loss_pred, frame_recall, doa_loss_pred_cnt,
                 nb_good_pks, more_est_cnt, less_est_cnt]
    return er_metric
...
def distance_between_gt_pred(gt_list_rad, pred_list_rad):
    ...
    ind_pairs = np.array([[x, y] for y in range(pred_len)
    ...
    if gt_len and pred_len:
        az1, ele1, az2, ele2 = gt_list_rad[ind_pairs[:, 0], 0], gt_list_rad[ind_pairs[:, 0], 1], \
            pred_list_rad[ind_pairs[:, 1],
                          0], pred_list_rad[ind_pairs[:, 1], 1]
...
def distance_between_gt_pred_xyz(gt_list, pred_list):
    ...
    if gt_len and pred_len:
        x1, y1, z1, x2, y2, z2 = gt_list[ind_pairs[:, 0], 0], gt_list[ind_pairs[:, 0], 1], gt_list[ind_pairs[:, 0], 2], \
            pred_list[ind_pairs[:, 1], 0], pred_list[ind_pairs[:, 1],
                                                     1], pred_list[ind_pairs[:, 1], 2]
        cost_mat[ind_pairs[:, 0], ind_pairs[:, 1]
...
def distance_between_spherical_coordinates_rad(az1, ele1, az2, ele2):
    ...
    """
    dist = np.sin(ele1) * np.sin(ele2) + np.cos(ele1) * \
        np.cos(ele2) * np.cos(np.abs(az1 - az2))
